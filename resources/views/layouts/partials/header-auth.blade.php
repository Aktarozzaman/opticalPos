	...
	<div class="row">
		<div class="col-md-6">
			<div class="pull-left mt-10">
		        <select class="form-control input-sm" id="change_lang" style=" border:none; border-radius:6px">
		...
		<div class="col-md-6">
			<div class="pull-right text-white margin">
		        ...
		        @if(!($request->segment(1) == 'business' && $request->segment(2) == 'register') && $request->segment(1) != 'login')
		        	 <a class="btn" href="{{ action('Auth\LoginController@login') }}@if(!empty(request()->lang)){{'?lang=' . request()->lang}} @endif">Login</a>
		        @endif
	        </div>
		</div>
	</div>

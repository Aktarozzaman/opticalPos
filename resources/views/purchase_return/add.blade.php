			              	...
			              	<td>
								...
								@isset($imei_by_keys[$purchase_line->id])
									@foreach($imei_by_keys[$purchase_line->id] as $itm)
										...
										<input type="hidden" name="all_imeis[]" value="{{$itm->id}}">
										
										<input class="form-check-input" {{ $itm->status == 1? 'checked':'' }}  type="checkbox" name="imeis[{{$purchase_line->id}}][{{$itm->id}}]" value="1" id="flexCheckDefault">

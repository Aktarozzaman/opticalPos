					...
					<table class="table bg-gray" id="sell_return_table">
				        ...
				        <tbody>
				            ...
				            <tr>
				              	...
				              	<td>
				                	{{ $sell_line->product->name }} @if($sell_line->imei) (IMEI / Serial : {{ $sell_line->imei->imei1 }}) @endif 
				              	...
				              	<td>
									...
									@if (!empty($sell_line->imei_ids))
									@php
										$imei_ids = json_decode($sell_line->imei_ids);

										
										$imeis = \App\Imei::whereIn('id',$imei_ids)->get();
										$loop_index = $loop->index;
									@endphp

									@foreach ($imeis as $imei_item)
									<input class="form-check-input"  type="checkbox" {{ $imei_item->status != 1? 'checked' : '' }}  name="products[{{$loop_index}}][imei_ids][]" value="{{ $imei_item->id }}" id="flexCheckDefault">
									<label class="form-check-label" for="flexCheckDefault">{{'IMEI1 : '.$imei_item->imei1.' '.'IMEI2 : '.$imei_item->imei2}}</label><br>
									@endforeach

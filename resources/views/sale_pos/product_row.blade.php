...
<tr class="product_row" data-row_index="{{ $row_count }}"
    ...
    <td>
        ...
        @php
            ...
            if (!empty($product->imei_id)) {
                $imei_text .= '<span class="p_imei_id" data-imei_id="imei_' . $product->imei_id . '"> IMEI-1 / SL-1: ' . $product->imei1 . ' IMEI-2 / SL-2: ' . $product->imei2 . ' <i class="fa fa-times text-danger cursor-pointer remove_imei" ></i><br/></span>';
            } elseif (!empty($product->imei_ids)) {
                $all_imeis = json_decode($product->imei_ids);
                foreach ($all_imeis as $imei_id) {
                    $im = \App\Imei::where('id', '=', $imei_id)->first();
                    if (!empty($im)) {
                        $imei_text .= '<span class="p_imei_id" data-imei_id="imei_' . $im->id . '"> IMEI-1 / SL-1: ' . $im->imei1 . ' IMEI-2 / SL-2: ' . $im->imei2 . ' <i class="fa fa-times text-danger cursor-pointer remove_imei" ></i><br/></span>';
                    }
                }
            }
        @endphp
        ...
        @if (($edit_price || $edit_discount) && empty($is_direct_sell))
            <div title="@lang('lang_v1.pos_edit_product_price_help')">
                <span class="text-link text-info cursor-pointer" data-toggle="modal"
                    ...
                    <span class="product_name_imei">{!! $product_name !!}</span>
        ...
        @else
            <span class="product_name_imei">{!! $product_name !!}</span>
        @endif
        <span class="imei_wrapper">{!! $imei_text !!}</span>
    ...
    <td>
        ...
        <span class="row_imei_input_wrapper" data-imei_name="products[{{ $row_count }}][imei_id][]">
            @if (!empty($product->imei_id))

                <input type="hidden" value="{{ $product->imei_id }}" id="imei_{{ $product->imei_id }}"
            ...
            @elseif(!empty($product->imei_ids))
                ...
                @foreach ($all_imeis as $imei__id)
                    <input type="hidden" value="{{ $imei__id }}" id="imei_{{ $imei__id }}"
                        name="products[{{ $row_count }}][imei_id][]" class="row_imei_id">
                @endforeach
        ...
        <div class="input-group input-number">
            @if (empty($product->imei_id) && empty($product->imei_ids))
            ...
            <input type="text" data-min="1" @if (!empty($product->imei_id) || !empty($product->imei_ids)) readonly @endif
                ...
                @if ($product->enable_stock && empty($pos_settings['allow_overselling']) && empty($is_sales_order)) data-rule-max-value="{{ $max_qty_rule }}" data-qty_available="{{ $product->qty_available }}"
    ...
    @else
        @if (!empty($warranties))
            {!! Form::select("products[$row_count][warranty_id]", $warranties, $warranty_id, [
                ...
                'class' => "form-control edit_warranty_$row_count",
            ]) !!}
        @endif
    ...
    <td class="{{ $hide_tax }}">
        <input type="text" name="products[{{ $row_count }}][unit_price_inc_tax]"
            ...
            @if (!empty($pos_settings['enable_msp'])) data-rule-min-value="{{ $unit_price_inc_tax }}"

$(document).ready(function() {
    ...
    $(document).on('change', '.purchase_quantity', function() {
        ...
        if(is_accessories==0){
            if(typeof $(this).data('quantity-value') == 'undefined'){
                // console.log('quantity : ' + quantity , 'product_id : ' + product_id , 'row : ' + row, 'variation_id : '+ variation_id)
    ...
    function add_imei(id, product_id, row, variation_id){
        ...
        for (let index = 0; index < id; index++) {
            $line +=`<div class="single_imei"><div><input type="text" name="imei1[`+product_id+`][`+variation_id+`][]"  placeholder="Enter IMEI 1 / Serial 1"></div>
            <div><input type="text" name="imei2[`+product_id+`][`+variation_id+`][]"  placeholder="Enter IMEI 2 / Serial 2"></div><br/><div>`
            
        }
        row.find('.imei').append($line);

        imeis = row.find('.imei').find('.single_imei');
    ...
    function add_imei_row(number_to_add, start_after, product_serial){
        ...
        for (let index = 0; index < number_to_add; index++) {
            ...
            $line +=`<div class="quantity_value_`+product_serial+`_`+start_after+`"><input type="text" name="imei1[`+product_serial+`][`+start_after+`][]"  placeholder="Enter IMEI 1 / Serial 1"></div>
            <div class="quantity_value_`+product_serial+`_`+start_after+`"><input type="text" name="imei2[`+product_serial+`][`+start_after+`][]"  placeholder="Enter IMEI 2 / Serial 2"></div><br/>`
        }
...
function set_supplier_address(data) {
    ...
    var supplier_name = data.text; // Extracting the name separately
    $('.supplier_name_div').find('.box-title').html('Supply Payment - '+supplier_name);

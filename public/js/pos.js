            ...
            .autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    if (ui.item.enable_stock != 1 || ui.item.qty_available > 0 || is_overselling_allowed) {
                        ...
                        pos_product_row(imei_info,ui.item.variation_id, purchase_line_id, null, 1, ui.item.imei_id, ui.item.imei1, ui.item.imei2);
                    } else {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        if (result.success == 1) {
                            ...
                            // if (result.receipt.is_enabled) {
                            //     enable_pos_form_actions();
                            //     var invoiceUrl = result.receipt.invoice_url + '?print_on_load=1';
...
function pos_product_row(imei_id= null, variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1, imei__id = null, imei__1 = null, imei__2 = null) {
        ...
        $('#pos_table tbody')
            ...
            .each(function() {
                ...
                if (
                    ...
                    if(imei__id != null){
                        $imei_names = '<span class="p_imei_id" data-imei_id="imei_'+imei__id+'"> IMEI-1 / SL-1: ' + imei__1+' IMEI-2 / SL-2: '+imei__2+' <i class="fa fa-times text-danger cursor-pointer remove_imei" ></i><br></span>'

                        $(this).find('.imei_wrapper').append($imei_names)

                        rowname = $(this).find('span.row_imei_input_wrapper');
                        $imei__input = '<input type="hidden" id="imei_'+imei__id+'" name="'+rowname.attr('data-imei_name')+'" value="'+imei__id+'">'
                        rowname.append($imei__input);
                    }
...
//Remove IMEI
$(document).on('click', '.remove_imei', function(){


    tr = $(this).closest('tr');

    let imei_id = ($(this).closest('span.p_imei_id').attr('data-imei_id'));



    qty_element = tr.find('.pos_quantity');
    var qty = __read_number(qty_element);
    if(qty>1){

        $(this).closest('span.p_imei_id').remove()
        $('input#'+imei_id).remove();
        __write_number(qty_element, qty - 1);
        qty_element.change();

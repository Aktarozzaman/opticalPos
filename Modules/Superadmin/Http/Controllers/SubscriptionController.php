...
class SubscriptionController extends BaseController
{
    ...
    public function index()
    {
        ...
        $current_package = Subscription::where('business_id', $business_id)
        ...
        return view('superadmin::subscription.index')
            ->with(compact('packages', 'current_package', 'active', 'nexts', 'waiting', 'permission_formatted', 'intervals'));
    }
    ...
    public function subscriptionCreate(Request $request)
    {
        ...
        if (!empty($package)) {
            ...
            if ($package_price <= $request->amount) {
                $paid_full = 'bkash';
            }
            ...
            $total_month = $request->month_number;
            $total_package_price = $request->final_amount;



            $this->_add_subscription($business_id, $package_id, $paid_full, $payment_transaction_id, $user_id, $total_month, $total_package_price);
    ...
    public function pay($package_id, $form_register = null)
    {
        ...
        try {
            ...
            $subcriptions_check = Subscription::where('business_id', $business_id)
                ->where('package_id', $package_id)
                ->where('payment_transaction_id', 'FREE')
                ->first();

            if ($subcriptions_check != null) {


                if ($package_id == 1) {

                    $enrolleSubscriber = Subscription::where('business_id', $business_id)
                        ->where('status', 'approved')
                        ->count();

                    if ($enrolleSubscriber) {

                        $output = ['success' => 0, 'msg' => __("You are already our valuable customer. Downgrading your package is not possible. Please contact Glorious POS Administrator for further assistance.")];
                        return redirect()
                            ->back()
                            ->with('status', $output);
                    }
                ...
                if ($current_package->package_id > $package_id) {
                    ...
                    $output = ['success' => 0, 'msg' => __("We're sorry, but you cannot downgrade your package at the moment. Please contact Glorious POS Administrator for further assistance.")];
                    return redirect()
                        ->back()
                        ->with('status', $output);
                }
            ...
            if ($package->price == 0) {
                ...
                $this->_add_subscription($business_id, $package, $gateway, $payment_transaction_id, $user_id, $total_month, $total_package_price);
    ...
    public function confirm($package_id, Request $request)
    {
        ...
        try {
            ...
            $total_month = $request->month_count;
            $total_package_price = $request->total_package_price;
            ...
            $this->_add_subscription($business_id, $package_id, request()->gateway, $payment_transaction_id, $user_id, $total_month, $total_package_price);
            DB::commit();
    ...
    public function postPaymentPaystackCallback()
    {
        ...
        if ($payment['status']) {
            ...
            $number = Contact::where('business_id', $business_id)
                    ->whereNotNull('mobile')
                    ->distinct()
                    ->pluck('mobile')
                    ->first();
    ...
    public function allSubscriptions()
    {
        ...
        return Datatables::of($subscriptions)
            ...
            ->addColumn('action', function ($row) {
                return '<a class="btn btn-primary btn-xs " href="' . action("\Modules\Superadmin\Http\Controllers\SubscriptionController@show_invoice", $row->id) . '" >View Invoice</a>';
            })
            ->addColumn('payment_status', function ($row) {
                return $row->payment_transaction_id ? '<span class="text-success">Paid</span>' : '<span class="text-danger">Unpaid</span>';
            })
            ->rawColumns(['package_price', 'action', 'payment_status'])
            ->make(true);
    }
    ...
    public function show_invoice($id)
    {
        ...
        $subscription = Subscription::where('subscriptions.business_id', $business_id)
            ...
            ->addSelect(
                ...
                'B.landmark as business_address'
            )->first();

        // dd($subscription);
        return view('superadmin::subscription.show_invoice', compact('subscription'));
    }


    public function paybkashpayment(Request $request)
    {
        // Retrieve form data

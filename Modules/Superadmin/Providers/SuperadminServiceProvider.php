    ...
    public function boot()
    {
        ...
        view::composer('superadmin::layouts.partials.active_subscription', function ($view) {
            ...
            if ($is_installed) {
                ...
                if ($current_trail_find === 1) {

                    $subCheck = Subscription::where('business_id', $business_id)
                        ->count();

                    if ($subCheck  > 1) {
                        $__subscription = Subscription::subscription_wise_active_subscription_($business_id);
                    } else {
                        $__subscription = Subscription::aasubscription_wise_active_subscription_($business_id);
                    }
                } else {
                    $__subscription = Subscription::subscription_wise_active_subscription_($business_id);
                }

...
class ProductUtil extends Util
{
     ...
     * @return boolean
     */
    public function createVariableProductVariations($product, $input_variations, $variation_name, $business_id = null)
    {
        // dd($product, $input_variations, $variation_name);
        if (!is_object($product)) {
            $product = Product::find($product);
        }
        ...
        foreach ($input_variations_edit as $key => $value) {
            ...
            if (!empty($value['variations'])) {
                foreach ($value['variations'] as $k => $v) {
                    ...
                    $NewVari = Variation::where('id', $v['id'])->first();
                    $NewVari->name = $v['value'];
                    $NewVari->default_purchase_price = $this->num_uf($v['default_purchase_price']);;
                    $NewVari->dpp_inc_tax = $this->num_uf($v['dpp_inc_tax']);
                    $NewVari->profit_percent =  $this->num_uf($v['profit_percent']);
                    $NewVari->default_sell_price = $this->num_uf($v['default_sell_price']);
                    $NewVari->sell_price_inc_tax = $this->num_uf($v['sell_price_inc_tax']);
                    $NewVari->save();
                    ...
                    Media::uploadMedia($product->business_id, $NewVari, request(), 'variation_images_' . $key . '_' . $k);
    ...
    public function decreaseProductQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0, $imei_id = null)
    {
        ...
        if (!empty($imei_id)) {
            ...
            foreach ($imei_id as $im_id) {
                Imei::where('id', '=', $im_id)->update(['status' => 1]);
            }
        }
    ...
    public function generateProductSku($string)
    {
        ...
        $product_number = TrackLastProductCode::where('business_id', $business_id)->first()->product_Last_id ?? 0;
        if ($product_number == 0) {
            $new_code = new TrackLastProductCode();
            $new_code->business_id = $business_id;
            $new_code->product_Last_id = $string;
            $new_code->save();
        } else {

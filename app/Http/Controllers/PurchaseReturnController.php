...
class PurchaseReturnController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $all_imeis = $request->input('all_imeis') ?? [];
            ...
            if (!empty($all_imeis)) {
                foreach ($all_imeis as $all_imei) {
                    Imei::where('id', $all_imei)->update(['status' => 0]);
                }
            }
            if ($imeis_ids) {
                foreach ($imeis_ids as $imei_id) {
            ...
            if (request()->ajax()) {
                ...
                if (empty($purchase_return->return_parent_id)) {
                    ...
                    foreach ($delete_purchase_lines as $purchase_line) {
                        ...
                        $imeis = Imei::where('business_id', $business_id)->where('purchase_line_id', $purchase_line->id)->get();
                        if (!empty($imeis)) {
                            foreach ($imeis as $imei) {
                                $imei->update(['status' => 0]);
                            }
                        }

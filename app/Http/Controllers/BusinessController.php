    ...
    */
    ...
    public function getRegister(Request $request)
    {
        ...
        if ($request->has('ref')) {
            ...
            $referrer = User::whereUsername($request->query('ref'))->first();
            $referrer->click_count = $referrer->click_count ? $referrer->click_count + 1 : 1;
            $referrer->save();
        }
    ...
    {
        $request['currency_id'] = '134';
        $request['country'] = 'Bangladesh';
        $request['time_zone'] = 'Asia/Dhaka';
        if (!config('constants.allow_registration')) {
            return redirect('/');
        }
        ...
        $request->validate([
            ...
            'alternate_number' => 'max:11',
        ]);

        //Create owner.
        $owner_details = $request->only(['first_name', 'nid', 'username', 'email', 'password', 'language']);

        $owner_details['language'] = empty($owner_details['language']) ? config('app.locale') : $owner_details['language'];


        $user = User::create_user($owner_details);
        $business_details = $request->only(['name', 'start_date', 'currency_id', 'time_zone', 'accounting_method']);
        $business_details['fy_start_month'] = 1;

        $business_location = $request->only(['name', 'website', 'mobile', 'alternate_number']);
        $business_location['landmark'] = $request['address_line_1'];
        $business_location['city'] = $request['address_line_2'];
        ...
        if (empty($business_details['start_date'])) {
            $business_details['start_date'] = today()->format('Y-m-d');
        } else {
            $business_details['start_date'] = Carbon::createFromFormat(config('constants.default_date_format'), $business_details['start_date'])->toDateString();
        }
        ...
        //upload license
        $license_name = $this->businessUtil->uploadFile($request, 'business_licese', 'business_liceses', 'image');
        if (!empty($license_name)) {
            $business_details['license'] = $license_name;
        }
        ...
        $client = new Client();
        $res = $client->request('POST', 'https://client.gloriousit.xyz/api/gloriouslab', [
            'form_params' => [
                "email" => $request->email,
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only([
                'name', 'sms_amount', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'default_profit_percent', 'default_sales_tax', 'default_sales_discount', 'sell_price_tax', 'sku_prefix', 'time_zone', 'fy_start_month', 'accounting_method', 'transaction_edit_days', 'sales_cmsn_agnt', 'item_addition_method', 'currency_symbol_placement', 'on_product_expiry',
            ...
            if ($request->currency_id == 1) {
                $custom_currency = Currency::find($request->currency_id);
                // Update the 'symbol' field with the new value here
                $custom_currency->update(['symbol' => $request->symbol]);
                // Save the changes to the database
                $custom_currency->save();

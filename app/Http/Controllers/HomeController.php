...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $birthDays = Contact::whereDate('dob', date('Y-m-d'))->count('id');

        $active = Subscription::active_subscription($business_id);
        $super_admin_notice = SuperadminNotice::first();

        $notice_message = '';

        if(!empty($super_admin_notice)){
            if( !empty($super_admin_notice->notice_message)){
                $notice_message .= $super_admin_notice->notice_message. ' . ';
            }
        }

        if (!empty($active)) {
            $givenDate = Carbon::parse($active->end_date);
            $currentDate = Carbon::now();

            if ($givenDate->isPast()) {
                //Days past
                $notice_message .= '<span class="text-danger">Your Subscription period is over. Please pay <span class="display_currency" data-currency_symbol="true">' . $active->package_price . '</span> to start using the software again.</span>';
            } elseif ($givenDate->isFuture()) {
                //Days left
                $daysLeft = $currentDate->diffInDays($givenDate, false);
                if ($daysLeft <= 5) {
                    $notice_message .= '<span class="text-danger">Only ' . $daysLeft . ' days left. Please pay <span class="display_currency" data-currency_symbol="true">' . $active->package_price . '</span> to continue using the software.</span>';
                }
            } else {
                $notice_message .= ' <span class="text-danger">Today is the last date of payment. Please pay <span class="display_currency" data-currency_symbol="true">' . $active->package_price . '</span> to continue using the software.';
            }
        }


        return view('home.index', compact( 'notice_message','product_labels', 'product_values', 'topContacts', 'birthDays', 'date_filters', 'sells_chart_1', 'sells_chart_2', 'widgets', 'all_locations', 'common_settings'));
    }

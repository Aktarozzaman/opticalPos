...
class LabelsController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        $barcode_settings = Barcode::where('business_id', $business_id)
            ...
            ->select(DB::raw('CONCAT(name, ", ", COALESCE(description, "")) as name, id, is_default,business_id'))
            ->get();
        $default = $barcode_settings->where('business_id',$business_id)->where('is_default', 1)->first();
        if ($default == null){

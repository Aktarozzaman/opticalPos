    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (auth()->user()->business_id =='1') {
                $menu->url(action('\Modules\Superadmin\Http\Controllers\SubscriptionController@index'), __('lang_v1.buy_package'), ['style' => 'font-size:21px !important; font-weight:bold!important; color:deeppink !important;', 'icon' => 'fa fa-shopping-cart'])->order(5);
            }
            $menu->url(action('HomeController@index'), __('home.home'), ['icon' => 'fa fas fa-fire', 'active' => request()->segment(1) == 'home'])->order(5);
                    ...
                    function ($sub) {
                        if (auth()->user()->can('user.view')) {
                            $sub->url(
                                ...
                                __('user.users'),
                                ['icon' => 'fa fas fa-users-cog', 'active' => request()->segment(1) == 'users']
                            );
                        }
            ...
            // // Trial User Role view
            if (auth()->user()->business_id =='1') {
                $menu->url(action('ManageUserController@trialuser'), __('lang_v1.trial_users'), ['icon' => 'fa fas fa-star', ])->order(5);
            }
            //Contacts dropdown
            if (auth()->user()->can('supplier.view') || auth()->user()->can('customer.view') || auth()->user()->can('supplier.view_own') || auth()->user()->can('customer.view_own')) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        $sub->url(
                            action('ContactController@birthDayList'),
                            __('lang_v1.birth_day_list'),
                            ['icon' => 'fa fas fa-users', 'active' => request()->segment(1) == 'customer-group']
                        );
            ...
            if (
                ...
                $menu->dropdown(
                    ...
                    ['icon' => 'fa fas fa-dolly', 'id' => 'tour_step5']
                )->order(20);
            }
            ...
            if (in_array('purchases', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create') || auth()->user()->can('purchase.update'))) {
                $menu->dropdown(
                    ...
                    ['icon' => 'fa fas fa-shopping-bag', 'id' => 'tour_step6']
                )->order(25);
            }
            //Sell dropdown
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update'])) {
                $menu->dropdown(
                    ...
                    ['icon' => 'fa fas fa-shopping-cart', 'id' => 'tour_step7']
                )->order(30);
            }
            ...
            if (auth()->user()->can('backup')) {
                $menu->url(action('BackUpController@index'), __('lang_v1.backup'), ['icon' => 'fa fas fa-database', 'active' => request()->segment(1) == 'backup'])->order(60);
            }

            //Modules menu
            if (auth()->user()->can('manage_modules')) {
                $menu->url(action('Install\ModulesController@index'), __('lang_v1.modules'), ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'manage-modules'])->order(60);
            }
            ...
            if (in_array('service_staff', $enabled_modules)) {
                $menu->url(action('Restaurant\OrderController@index'), __('restaurant.orders'), ['icon' => 'fa fas fa-calendar', 'active' => request()->segment(1) == 'modules' && request()->segment(2) == 'orders'])->order(75);
            }
            ...
            // $menu->url(action('NotificationSmsController@index'),
            //                     __('SMS Notification'),
            //                     ['icon' => 'fa fas fa-sms', 'active' => request()->segment(1) == 'smsnotification'])->order(85);
            ...
            $menu->dropdown(
                ...
                function ($sub) {
                    ...
                    $sub->url(
                        action('MessageController@sendSms'),
                        __('lang_v1.send_sms'),
                        ['icon' => 'fa fas fa-book', 'active' => request()->segment(1) == 'message' && request()->segment(2) == 'send-sms']
                    );

                    $sub->url(
                        action('MessageController@smsTemplate'),
                        __('lang_v1.template'),
                        ['icon' => 'fa fas fa-exchange-alt', 'active' => request()->segment(1) == 'message' && request()->segment(2) == 'sms-template']
                    );

                    $sub->url(
                        action('MessageController@groupSms'),
                        __('lang_v1.group_sms'),
                        ['icon' => 'fa fas fa-file-alt', 'active' => request()->segment(1) == 'message' && request()->segment(2) == 'group-sms']
                    );

                    $sub->url(
                        action('MessageController@messageHistory'),
                        __('lang_v1.sms_histry'),
                        ['icon' => 'fa fas fa-balance-scale', 'active' => request()->segment(1) == 'message' && request()->segment(2) == 'message-history']
                    );

                    $sub->url(
                        action('MessageController@index'),
                        __('lang_v1.sms_setting'),
                        ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'message' && request()->segment(2) == 'message']
                    );


                    $sub->url(
                        action('MessageController@buySms'),
                        __('lang_v1.buy_sms'),
                        ['icon' => 'fa fas fa-file-alt', 'active' => request()->segment(1) == 'message' && request()->segment(2) == 'buy-sms']
                    );
                    // $sub->url(
                    //     action('ManageUserController@demouser'),
                    //     __('Demo Page'),
                    //     ['icon' => 'fa fas fa-file-alt', 'active' => request()->segment(1) == 'message' && request()->segment(2) == 'buy-sms']
                    // );
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules) {
                        if (auth()->user()->can('business_settings.access')) {
                            ...
                            // $sub->url(
                            //     action('NotificationSmsController@index'),
                            //     __('SMS Notification'),
                            //     ['icon' => 'fa fas fa-map-marker', 'active' => request()->segment(1) == 'smsnotification']
                            // );
                            $sub->url(

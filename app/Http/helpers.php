...
function getImeiText($imei_id, $show_imei_1 = 1, $show_imei_2 = 1){
    $text='';
    if(!empty($imei_id)){
        if($show_imei_1 == 1 || $show_imei_2 == 1){
            foreach($imei_id as $im_id){
                $im = \App\Imei::where('id','=',$im_id)->first();
    
                $text .= '(';
                if($im){
                    if($show_imei_1 == 1){

                        $text .= 'IMEI-1 / SL-1: '.$im->imei1;
                    }
                    if($show_imei_2 == 1){
                        $text .=' IMEI-2 / SL-2: '.$im->imei2;
                    }
                }
                $text .= ')';
            }
